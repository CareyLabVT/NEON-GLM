...
# Mendota Example
lakeShape = readOGR('./lake_shape/BARC/D03BARC_BATH_20180418_contour1m.shp',layer='D03BARC_BATH_20180418_contour1m')
proj4string(lakeShape)
extent = as.numeric(lakeShape@bbox)
# Extent for BARCO
extent = c(-82.0106795452453,  29.67444621618174, -82.00661916405241,  29.67795510253453)

###########################################################
### Set timeframe
###########################################################
out = seq.POSIXt(as.POSIXct('2018-10-28 02:00',tz = 'GMT'),as.POSIXct('2020-08-31 23:00',tz='GMT'),by = 'hour')
...
###########################################################
cellNum = 8 #How many output cells will there be? Need to check this beforehand
for (l in 1:11){
...
setwd("D:/RYAN/NLDAS_NEON/BARC_SUGG/")
ptm <- proc.time()
...
for (f in 1:11){
  write.csv(output[[f]],paste('BARCO_SUGGS_',vars[f],'.csv',sep=''),row.names=F)
}
